    ...
    <>
      ...
      <nav className="fixed top-4 left-1/2 z-50 -translate-x-1/2 flex md:hidden w-full justify-center">
        <button
          ...
          className="bg-white/80 rounded-full shadow-xl p-3 border border-blue-100 flex items-center justify-center focus:outline-none"
        ...
        {open && (
          <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 bg-transparent backdrop-blur-lg animate-fade-in overflow-hidden w-screen h-screen">
            <div className="bg-white rounded-2xl shadow-2xl border border-blue-100 flex flex-col items-center gap-4 px-8 py-8 min-w-[70vw] max-w-xs">
              {navItems.map((item) => (
                <Link
                  key={item.path}
                  href={item.path}
                  className={`w-full text-center px-4 py-3 rounded-xl font-semibold text-lg transition-all duration-200
                    ${pathname === item.path ? "bg-blue-200 text-blue-900" : "text-blue-700 hover:bg-blue-100"}
                  `}
